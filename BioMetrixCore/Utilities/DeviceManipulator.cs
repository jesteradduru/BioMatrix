...
namespace BioMetrixCore
{
    internal class DeviceManipulator
    {
        ...
        public ICollection<MachineInfo> GetLogData(ZkemClient objZkeeper, int machineNumber)
        {
            ...
            IDictionary<int, List<int>> userIDCount = new Dictionary<int, List<int>>();
            ...
            while (objZkeeper.SSR_GetGeneralLogData(machineNumber, out dwEnrollNumber1, out dwVerifyMode, out dwInOutMode, out dwYear, out dwMonth, out dwDay, out dwHour, out dwMinute, out dwSecond, ref dwWorkCode))
            {
                ...
                //Input Interval - Earliest in Time In (--10:59), First Two inputs in BREAK (11-3), Earliest in Time Out (3:01++)

                if (!userIDCount.ContainsKey(objInfo.IndRegID))
                    userIDCount.Add(objInfo.IndRegID, new List<int> { dwMonth, dwDay, 0,0,0,0 });
                
                if (userIDCount[objInfo.IndRegID][0] != dwMonth || userIDCount[objInfo.IndRegID][1] != dwDay)
                {
                    userIDCount[objInfo.IndRegID][0] = dwMonth;
                    userIDCount[objInfo.IndRegID][1] = dwDay;
                    userIDCount[objInfo.IndRegID][2] = 0; // Time IN
                    userIDCount[objInfo.IndRegID][3] = 0; // Break OUT
                    userIDCount[objInfo.IndRegID][4] = 0; // Break IN
                    userIDCount[objInfo.IndRegID][5] = 0; // Time OUT
                }

                bool currentDay = userIDCount[objInfo.IndRegID][0] == dwMonth && userIDCount[objInfo.IndRegID][1] == dwDay;

                if (dwHour < 11 && userIDCount[objInfo.IndRegID][2] == 0 && currentDay)
                {
                    objInfo.Remarks = "Time IN";
                    userIDCount[objInfo.IndRegID][2] = 1;
                }
                else if (dwHour >= 11 && (dwHour < 15 || (dwHour == 15 && dwMinute == 0)) && currentDay)
                {
                    if (userIDCount[objInfo.IndRegID][2] == 1 && userIDCount[objInfo.IndRegID][3] == 0)
                    {
                        objInfo.Remarks = "Break OUT";
                        userIDCount[objInfo.IndRegID][3] = 1;
                    }
                    else if (userIDCount[objInfo.IndRegID][4] == 0)
                    {
                        objInfo.Remarks = "Break IN";
                        userIDCount[objInfo.IndRegID][4] = 1;
                    }
                }
                else if (((dwHour == 15 && dwMinute > 0) || dwHour > 15) && currentDay && userIDCount[objInfo.IndRegID][5] == 0)
                {
                    objInfo.Remarks = "Time OUT";
                    userIDCount[objInfo.IndRegID][5] = 1;
                } 
